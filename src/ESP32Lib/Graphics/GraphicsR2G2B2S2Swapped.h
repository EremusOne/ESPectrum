...
class GraphicsR2G2B2S2Swapped: public Graphics<unsigned char>
{
	...
	{
		frameBuffers[y][x^2] = (color & RGBAXMask) | SBits;
	}
	...
	virtual void dotAdd(int x, int y, Color color)
	{
		if ((unsigned int)x < xres && (unsigned int)y < yres)
		{
			...
			frameBuffers[y][x^2] = r | (g & 0b1100) | (b & 0b110000) | SBits;
		}
	}
	...
	virtual void dotMix(int x, int y, Color color)
	{
		if ((unsigned int)x < xres && (unsigned int)y < yres && (color >> 6) != 0)
		{
			...
			unsigned int co = frameBuffers[y][x^2];
			...
			frameBuffers[y][x^2] = ((r | g | b) >> 16) | SBits;
		}	
	}
	...
	{
		if ((unsigned int)x < xres && (unsigned int)y < yres)
			return frameBuffers[y][x^2] & RGBAXMask;
		return 0;
	}
